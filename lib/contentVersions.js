...
const FIELDS = {
  post: ['title', 'slug', 'excerpt', 'content', 'cover_image', 'tags', 'status'],
...
function saveVersion(db, entityType, entityId, snapshot, source = 'save') {
  ...
  const result = db.prepare(`
    INSERT OR IGNORE INTO content_versions
      (entity_type, entity_id, source, snapshot_json, content_hash)
    VALUES (?, ?, ?, ?, ?)
  `).run(entityType, entityId, source, json, hash);
...
function listVersions(db, entityType, entityId, limit = 50) {
  ...
  return db.prepare(`
    SELECT id, entity_type, entity_id, source, snapshot_json, content_hash, created_at
