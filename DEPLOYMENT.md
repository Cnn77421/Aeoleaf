...
1. 登录 `/admin/security`，确认当前设备、IP 和最后活动时间正确；
2. 重新验证管理员密码，添加一个通行密钥；
3. 在无痕窗口使用通行密钥登录；
4. 撤销该无痕窗口的会话，确认其下一次请求回到登录页；
5. 保留密码登录入口，避免设备遗失后无法进入后台。

升级前产生的旧会话没有设备元数据，会在安全中心显示为“旧版会话”。可以使用“退出其他设备”一次性清理。
...
- 预约发布和自动下线由应用内每分钟任务执行，部署后需保持 PM2 进程常驻。修改系统时区后应重启应用；后台时间按服务器本地时区保存和执行。
- 预览链接使用 `SESSION_SECRET` 签名，有效期 1 小时。轮换 `SESSION_SECRET` 会立即使已有预览链接和登录会话失效。
- 发布后检查 `/sitemap.xml` 与 `/rss.xml`；标记为 `noindex` 的文章不会出现在其中。
- 图片优化依赖 Sharp 0.35.3 或更高版本。`npm ci --omit=dev` 后可执行 `node -e "console.log(require('sharp').versions)"` 确认 libvips 已正确加载。
- 媒体优化只创建带 `-optimized-<时间>.webp` 的新副本，不覆盖原图。替换内容引用并确认页面正常后，再从媒体库删除未引用的旧文件。

## 八、P2 通知、健康检查与高级分析验收

- 应用启动后立即执行一次系统健康检查，之后每 5 分钟执行一次；只在状态发生变化时生成通知，避免重复告警。健康页保留最近 500 次快照。
- 磁盘空间低于 15% 或 2GB 时提示警告，低于 5% 或 512MB 时提示错误。生产环境应为数据库目录预留足够空间。
- 健康检查会验证 `DATABASE_PATH`、`PUBLIC_DIR/uploads` 和 `BACKUP_DIR`。若使用自定义目录，确认运行用户拥有读写权限。
- 通知中心会收集失败认证、备份失败、预约发布/下线、待审核留言和健康状态变化。部署后分别触发一项低风险测试，确认侧栏与顶栏未读徽标同步更新。
- 高级访客分析依赖访客的 `fingerprint_id`、`visitor_session_id`、UTM 和阅读行为字段。刚部署或数据量很少时，留存、漏斗和路径转移为空属于正常现象。
- `/admin/visitors/advanced` 提供 7、30、90 天周期对比；上线后确认机器人流量已被 `is_bot` 排除，并抽查一条 UTM 链接的来源归因。
